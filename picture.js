window.onload = function(){
    let box=this.document.getElementsByClassName("re")[0];
    let lik=box.getElementsByTagName("li");
    ...
    timer=this.setInterval(auto,1800);
    box.onmouseover = function () { //鼠标在图片上，停止轮播
        console.log('good');
        clearInterval(timer);
    }
    box.onmouseout = function () { //鼠标划出，继续轮播
         timer = setInterval(auto, 1800); //调用定时器
    }
    let j =0;
